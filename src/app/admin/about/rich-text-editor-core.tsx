 ...
 * ### Safety
 *
 * Tiptap only emits nodes/marks from the configured schema, so the HTML
 ...
 *
 * Loaded only from `rich-text-editor.tsx` via `next/dynamic` so Tiptap is a
 * separate async chunk (INF-106).
 */
...
const RichTextEditorCoreImpl = forwardRef<
  RichTextEditorHandle,
  RichTextEditorCoreProps
>(function RichTextEditorCore(
  { initialHtml, placeholder, onDirty },
  ref,
) {
...
RichTextEditorCoreImpl.displayName = "RichTextEditorCore";

/** Editable Tiptap instance with a static toolbar. Parent reads HTML via ref. */
export const RichTextEditorCore = memo(RichTextEditorCoreImpl);
